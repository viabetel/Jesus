...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    setReturnTo(next.startsWith("/") ? next : "/minha-conta")
  }, [])
  ...
  }, [isHydrated, isAuthenticated, returnTo])

  if (!isHydrated || redirecting) {
    return <AuthLoadingScreen message="Carregando sua conta..." />
  }
  ...
  return (
    <div className="flex min-h-dvh">
      <div className="flex w-full flex-col justify-center px-6 py-12 sm:px-10 lg:w-1/2 lg:px-16 xl:px-24">
        ...
        <p className="mt-2 text-[14px] text-[var(--muted-foreground)]">Acesse sua conta para acompanhar pedidos e favoritos.</p>
        <form onSubmit={handleSubmit} className="mt-8 space-y-5 max-w-md">
          <div>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="seu@email.com" autoComplete="email" autoFocus
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[var(--muted-foreground)]">
      ...
      <div className="hidden bg-[var(--ink)] lg:flex lg:w-1/2 lg:items-center lg:justify-center">
        <div className="text-center px-12">
          <div className="mx-auto h-24 w-24 rounded-full bg-white/5 flex items-center justify-center mb-8"><div className="h-12 w-12 bg-white/10 rounded" /></div>
          <h2 className="font-serif text-white text-[28px] font-bold">Bem-vindo de volta</h2>
          <p className="mt-3 text-white/60 text-[14px] max-w-sm mx-auto leading-relaxed">Entre na sua conta para acompanhar pedidos, salvar favoritos e aproveitar ofertas exclusivas.</p>
        </div>
      </div>
    </div>
  )

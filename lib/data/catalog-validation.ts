...
export function validateProduct(p: Product): Issue[] {
  ...
  const validImages = p.images.filter(url => !!url)
  if (!validImages.length) add("error", "NO_IMAGES", "Sem imagens")
  else if (validImages.length < 2) add("warning", "FEW_IMAGES", "Apenas 1 imagem")

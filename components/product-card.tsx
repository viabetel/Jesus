...
// Supports both PublicProduct (with coverImage/hoverImage) and legacy Product
...
export function ProductCard({ product, featured = false }: { product: CardProduct; featured?: boolean }) {
  ...
  return (
    <Link href={`/produto/${product.slug}`} className="group block" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      {/* Image */}
      <div className={cn("relative mb-2 overflow-hidden bg-[#f6f2ea]", featured ? "aspect-[3/4] rounded-lg" : "aspect-[4/5] rounded-md")}>
        ...
        {hasHoverImage && (
          <Image src={hoverImg!} alt={`${product.name} — hover`} fill
            className={cn("object-cover transition-all duration-500", hovered ? "opacity-100" : "opacity-0 scale-95")}
            sizes={featured ? "(max-width:640px) 100vw, 50vw" : "(max-width:640px) 50vw, 25vw"} />
        )}
        ...
        {product.badge && (
          <span className={cn("absolute left-2 top-2 rounded-sm px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider sm:text-[9px]",
            product.badge === "Promoção" ? "bg-red-600 text-white" : "bg-foreground text-background"
          )}>
        ...
        {/* Hover CTA */}
        <div className={cn("absolute bottom-0 left-0 right-0 flex items-center justify-center pb-3 transition-all duration-300",
          hovered && !isOutOfStock ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
        )}>
      ...
      <div className="space-y-0.5 px-0.5">
        ...
        <h3 className="line-clamp-1 text-xs font-medium leading-snug sm:text-[13px]">{product.name}</h3>
        ...
        {totalStock > 0 && totalStock <= 5 && <p className="text-[8px] font-medium text-amber-600">Últimas {totalStock} un.</p>}
      </div>
    </Link>
  )
}
